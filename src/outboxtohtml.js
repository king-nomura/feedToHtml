import { OutboxParser } from './services/outbox_parser.js';
...
/**
 * Main OutboxToHtml API class - facade for all ActivityPub outbox to HTML conversion functionality
 * Outputs are organized by month: YYYY/YYYY-MM.html
 */
export class OutboxToHtml {
  constructor(config = {}) {
    // Initialize services (except TemplateEngine which needs config)
    this.outboxParser = new OutboxParser();
  ...
  /**
   * Convert ActivityPub outbox to HTML files organized by month
   * @param {string} outboxUrl - Outbox URL
   * @param {Object} [options] - Conversion options
   * @param {string} [options.template] - Template file path
   * @param {string} [options.outputDir] - Output directory
   * @param {boolean} [options.verbose] - Enable verbose logging
   * @param {string} [options.filePath] - Local JSON file path (alternative to URL)
   * @param {string} [options.title] - Override feed title
   * @param {string} [options.description] - Override feed description
   * @returns {Promise<Object>} Conversion result
   */
  async convert(outboxUrl, options = {}) {
    try {
      // Merge options with configuration
      const mergedConfig = this.configLoader.mergeConfigs(this.config.toJSON(), options);
      const source = options.filePath || outboxUrl;
      ...
      if (options.filePath) {
        ...
        if (options.verbose) {
          ...
          console.log(`Loading outbox from file: ${options.filePath}`);
        }

        feed = this.outboxParser.parseFromFile(options.filePath, {
          title: options.title,
          description: options.description
        });
      } else {
        // Validate URL (basic validation for ActivityPub)
        try {
          new URL(outboxUrl);
        } catch (error) {
          throw new Error(`CONFIG Invalid outbox URL: ${outboxUrl}`);
        }

        if (options.verbose) {
          console.log(`Configuration: ${this.configLoader.getConfigSummary(mergedConfig)}`);
          console.log(`Processing ActivityPub outbox: ${outboxUrl}`);
          console.log('Fetching outbox...');
        }

        feed = await this.outboxParser.fetchAndParse(outboxUrl, mergedConfig.timeout);
      }
      this.outboxParser.validateFeed(feed);
      ...
      if (options.verbose) {
        console.log(`Outbox loaded: "${feed.title}" (${feed.getItemCount()} items)`);
      }
  ...
  /**
   * Convert ActivityPub outbox to HTML string (in-memory)
   * @param {string} outboxUrl - Outbox URL
   * @param {Object} [options] - Conversion options
   * @returns {Promise<Object>} Conversion result with HTML content
   */
  async convertToString(outboxUrl, options = {}) {
    try {
      // Validate URL
      try {
        new URL(outboxUrl);
      ...
      // Fetch and parse outbox
      const feed = await this.outboxParser.fetchAndParse(outboxUrl, mergedConfig.timeout);
      this.outboxParser.validateFeed(feed);
  ...
  /**
   * Validate ActivityPub outbox without conversion
   * @param {string} outboxUrl - Outbox URL
   * @param {Object} [options] - Validation options
   * @returns {Promise<Object>} Validation result
   */
  async validateOutbox(outboxUrl, options = {}) {
    try {
      // Validate URL
      try {
        new URL(outboxUrl);
        ...
        return {
          ...
          url: {
            isValid: false,
            errors: [`Invalid URL: ${outboxUrl}`],
            warnings: []
          }
        };
      }

      // Fetch and parse outbox (with shorter timeout for validation)
      const timeout = Math.min(options.timeout || 30000, 30000);
      const feed = await this.outboxParser.fetchAndParse(outboxUrl, timeout);
      this.outboxParser.validateFeed(feed);
  ...
  /**
   * Create OutboxToHtml instance with default configuration
   * @param {Object} [options] - Configuration options
   * @returns {OutboxToHtml} New instance
   */
  static create(options = {}) {
    return new OutboxToHtml(options);
  }
}
